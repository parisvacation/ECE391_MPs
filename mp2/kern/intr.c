...
void intr_handler(int code) {
    ...
    // Input:
    //      code -- the interrupt source code
    // Output:
    //      None
    // Side effect:
    //      The function serve as the interrupt handler, including the external interrupt and timer interrupt;

    switch (code) {
    // Interrupt raiseed by external factor
    case RISCV_MCAUSE_EXCODE_MEI:
        extern_intr_handler();
        break;
    // Interrupt raised by time factor
...
void extern_intr_handler(void) {
    ...
    // Dispatch interrupt to appropriate ISR(Call the function)
